...
app.add_middleware(
    ...
    allow_origins=[
        ...
        # "http://localhost:3000",  # <- add for local dev if needed
    ],
...
def _lookup_account_conf(tenant_id: str, account_ref: str | None) -> dict:
    """
    Supports both new multi-account shape TENANTS[tenant]['accounts'][account_ref]
    and legacy single-account fields on the tenant.
    """
    t = TENANTS.get(tenant_id) or {}
    if account_ref and (t.get("accounts") or {}).get(account_ref):
        return t["accounts"][account_ref]
    # legacy fields expected at tenant root
    legacy = {
...
def list_aws_regions(
    ...
    """
    Returns a list of region codes. Falls back to a static list if AWS call fails.
    """
    cache_key = f"{tenant_id}:{account_ref}:{include_opt_in}"
    ...
    # Try dynamic
    try:
        # use us-east-1 to list regions (doesn't matter much)
...
def list_instance_types(
    ...
    """
    Returns available instance types in a region. If tenant/account not supplied or AWS fails,
    we return a curated general-purpose list so the UI still works.
    """
    cache_key = f"{tenant_id}:{account_ref}:{region}:{family_prefix or '*'}"
    cached = _cache_get(_META_CACHE["instance_types"], cache_key)
    if cached:
        return {"instance_types": cached, "source": "cache"}

    # Try dynamic (describe_instance_type_offerings)
    try:
        types: Set[str] = set()
        if tenant_id:
    ...
    # Static curated list (covers common families)
    static = [
